...
def load_data(img_path, ratio, aug, index, kernel_path='ground_truth_npy'):
    try:
        gt_path = img_path.replace('.jpg', '.npy').replace('images', kernel_path)
        target = np.load(gt_path)
    except:
        gt_path = img_path.replace('.png','.npy').replace('images', kernel_path)
        target = np.load(gt_path)
...
class RawDataset(Dataset):
    def __init__(self, root, transform, ratio=8, aug=False, kernel_path='ground_truth_npy'):
    ...
    def __getitem__(self, index):
        img, target = load_data(self.root[index], self.ratio, self.aug, index, self.kernel_path)
        if self.transform:
    ...
    def avg_count(self):
        count = []
        for img in  self.root:
            #print(img)
            try:
                count.append(np.load(img.replace('.png','.npy').replace('images', self.kernel_path)).sum())
            except:
                count.append(np.load(img.replace('.jpg','.npy').replace('images', self.kernel_path)).sum())
        print(np.array(count).mean())
        print(np.array(count).std())
        print(np.median(np.array(count)))
        median = np.median(np.array(count))
        #print(np.median(abs(np.array(count)-median)))
        hist = np.histogram(np.array(count),int(round(sqrt(self.nsamples))))
        print(hist[0])
        print(hist[1])
        print(np.array(count).max())
        print(np.array(count).min())
